...
Entry *lookUpCond(char *s, Table *tableP, int checkStars) {
    ...
    if (strcmp(tableP->scope, "Globale") != 0)
        return lookUpCond(s, getGlobale(), checkStars);
    else
        return NULL;
}
...
void initTable() {
    ...
    table->scope = newstring(
            "Globale"); //Verra' riscritto solo se e' una funzione, metodo non pulitissimo si puo' rivedere
    table->count = 0;
    int i;
    for (i = 0; i < TOT; i++)
        table->entry[i] = NULL;
